...
class TemperatureMachine(StateMachine):
    ...
    def updateLights(self):
        ...
        ## LED behavior in heat, cool and off states
        # Heat
        if self.current_state == self.heat:
            if round(self.getFahrenheit()) < self.setPoint:
                redLight.pulse()
            elif round(self.getFahrenheit()) >= self.setPoint:
                redLight.on()
        # Cool
    ...
    def setupSerialOutput(self):
        ## String with current state, current temp, current setpoint
        output = f"S:{self.current_state.id}, T:{self.getFahrenheit():0.3f}, SP:{self.setPoint}"
    ...
    def manageMyDisplay(self):
        ...
        while not self.endDisplay:
            ...
            if(altCounter < 6):
                ...
                ## Current temp only
                lcd_line_2 = f"Temp:{round(self.getFahrenheit())}F"
    
                altCounter = altCounter + 1
            else:
				## Current state and setpoint
                lcd_line_2 = f"State:{self.current_state.id} Set:{self.setPoint}"
